...
public class SoundManager {
	 ...
	 * @return
	 */
	public static final int sendSound(Client p, int id, SoundType type) {
		switch (type) {
		/**
		 * RANGE COMBAT SOUNDS so basically if i want to add new bow soudn
		 */
		case RANGED_COMBAT:
			String bow = ItemAssistant.getItemName(id).toLowerCase();
		...
		case PLAYER_BLOCK:
			...
			String shield = ItemAssistant.getItemName(id).toLowerCase();
	 ...
	 * @param id
	 */
	public static final void play(Client player, int i) {
		if (!Config.SOUND)
			return;

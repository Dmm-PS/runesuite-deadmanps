...
public class Clan {
	...
	public void addMember(Client player) {
		if (isBanned(player.playerName)) {
			player.sendMessage("@red@You are currently banned from this clan chat.@bla@");
			return;
		}
		...
		player.getPA().sendString(
				"Talking in: @red@" + getTitle() + "@bla@", 18139);
		player.getPA().sendString(
				"Owner: @red@" + Misc.formatPlayerName(getFounder())
						+ "@bla@", 18140);
		player.sendMessage("Now talking in clan chat @red@"
				+ getTitle() + "@bla@.");
		player.sendMessage("To talk, start each line of chat with the @red@/@bla@ symbol.");
		updateMembers();
	}
	 ...
	 * @param message
	 */
	public void sendChat(Player player, String paramString) {
		if (getRank(player.playerName) < this.whoCanTalk) {
			((Client) player).sendMessage("Only " + getRankTitle(this.whoCanTalk)

package wind.model.players.content.skills.impl.farming;
...
public class Farming extends SkillHandler{
    ...
    public final static int PATCH_WEEDS = 8389;
 ...
 /* seed watering, cool */
    ...
    private void cleanPatch() {
        if (!patchCleaned) {
            c.getPA().object(PATCH_CLEAN, 3058, 3311, -1, 10);
            patchCleaned = true;
        } else {
    ...
    public void updateHerbPatch() {
        if (c.farm[0] > 0 && c.farm[1] > 0) {
            c.getPA().object(PATCH_HERBS, 3058, 3311, -1, 10);
        } else {
            c.getPA().object(PATCH_WEEDS, 3058, 3311, -1, 10);
            patchRaked = false;
            seedWatered = false;
            seedPlanted = false;
            patchCleaned = false;

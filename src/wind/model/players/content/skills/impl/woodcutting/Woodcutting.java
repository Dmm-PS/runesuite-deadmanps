...
import wind.model.players.content.randomevents.SpiritTree;
...
public class Woodcutting extends SkillHandler {
		...
		for (int i = 0; i < trees.length; i++) {
			for (int l = 0; l < 6; l++)
				if (tree == trees[l][i]) {
					...
					TaskHandler.submit(new Task(getTimer(c), false) {
	...
	private static void createStump(final Client c, final int i, final int obX,
		...
		TaskHandler.submit(new Task(getRespawnTime(c, i), true) {
			...
			@Override
			public void execute() {
				ObjectHandler.createAnObject(c, i, obX, obY);
				this.cancel();
			}
	...
	private static boolean performCheck(Client c, int itemID, int level) {
		return (c.getItems().playerHasItem(itemID) || c.playerEquipment[3] == itemID)
				&& c.playerLevel[8] >= level;
	}
	...
	private static int[][] trees = {
			...
			{ // OAK
			1281, 11756, 3037, 8462, 8463, 8464, 8465, 8466, 8467, 10083, 13413,
					13420, },
			...
			{ // MAPLE
			1307, 11762, 4674, 8435, 8436, 8437, 8438, 8439, 8440, 8441, 8442, 8443,
					8444, 8454, 8455, 8456, 8457, 8458, 8459, 8460, 8461,
					13415, 13423, },
			{ // YEW
			1309, 11758, 8503, 8504, 8505, 8506, 8507, 8508, 8509, 8510, 8511, 8512,
					8513, 13416,  13422, },
			{ // MAGIC
			1306, 11764, 8396, 8397, 8398, 8399, 8400, 8401, 8402, 8403, 8404, 8405,

...
public class CastleWars {
	 ...
	 * Game timers.
	 */
	private static final int GAME_TIMER = 500; // 1500 * 600 = 900000ms = 15
												// minutes
	...
	public static boolean zammySideDoor, zammyRock1, zammyRock2, zammyCatapult,
			saraSideDoor, saraRock1, saraRock2, saraCatapult;
		...
		TaskHandler.submit(new Task(2, true) {
			...
			public void execute() {
				if (c.getItems().addItem(item, 1)) {
					c.getItems();
					c.getItems();
					c.sendMessage("You take a "
							+ ItemAssistant.getItemName(item).toLowerCase()
							+ ".");
				}
				this.cancel();
			}
	...
	public static void toWaitingRoom(Client player, int team) {
		int[] food = { 391, 385, 379, 333, 329, 373, 361, 7946, 397, 1891, 365,
				...
				6703, 1963, 6883, 2108, 2118, 2116, 15272 };
		...
		if (team == 1) {
			...
			if (getZammyPlayers() >= getSaraPlayers() || getSaraPlayers() == 0) {
				...
				player.sendMessage("Next Game Begins In:@red@ "
						+ (((gameStartTimer * 3) + 3) + (timeRemaining * 3))
						+ " @bla@seconds.");
	...
	public static void returnFlag(Client player, int wearItem) {
		...
		player.getEquipment().updateSlot(3);
	...
	public static void addFlag(Client player, int flagId) {
		...
		player.getEquipment().updateSlot(player.playerWeapon);
		player.appearanceUpdateRequired = true;
		Player.updateRequired = true;
	}
	...
	public static void dropFlag(Client player, int flagId) {
		...
		player.appearanceUpdateRequired = true;
		Player.updateRequired = true;
		new Object(object, player.getX(), player.getY(), player.heightLevel, 0,
				10, object, -1, 1);
	}
	...
	public static void pickupFlag(Client player) {
		...
		while (iterator.hasNext()) {
			...
			teamPlayer.getPA().createObjectHints(player.objectX,
					player.objectY, 170, -1);
			new Object(-1, player.objectX, player.objectY, player.heightLevel,
					0, 10, -1, -1, 1);
		}
		return;
	}
		...
		while (iterator.hasNext()) {
			...
			if (player != null) {
				player.getPA().sendFrame126(
						"Next Game Begins In:@red@ "
								+ ((gameStartTimer * 3) + (timeRemaining * 3))
								+ " @whi@seconds.", 6570);
				player.getPA()
						.sendFrame126(
								"Zamorak Players: @red@" + getZammyPlayers()
										+ "@whi@.", 6572);
				player.getPA().sendFrame126(
						"Saradomin Players: @blu@" + getSaraPlayers()
								+ "@whi@.", 6664);
	...
	public static boolean attackDoor(final Client c, final int object) {
		...
		if (c.isAttackingGate)
			return true;
		if (object != 4423 && object != 4424 && object != 4427
				&& object != 4428) {
			return false;
		}
		if ((object == 4423 || object == 4424) && getTeamNumber(c) == 1
				|| (object == 4427 || object == 4427) && getTeamNumber(c) == 2) {
		...
		c.getItems();
		TaskHandler.submit(new Task(c.getCombat().getAttackDelay(
				ItemAssistant
			...
			public void execute() {
				...
				int damage = 0;
				if (c.usingSpecial && !c.autocasting) { // special
														// attack
				...
				if (c.autocasting) { // magic
					...
					if (c.MAGIC_SPELLS[c.spellId][4] > 0) {
						c.getPA().createPlayersProjectile(pX, pY, offX,
								offY, 50, 78,
								c.MAGIC_SPELLS[c.spellId][4],
								c.getCombat().getStartHeight(),
								c.getCombat().getEndHeight(), 0,
								c.getCombat().getStartDelay());
					}
					damage = Misc
							.random(c.MAGIC_SPELLS[c.oldSpellId][6]);
					// damage =
					// c.getCombat().getHit(MagicMaxHit.magicMaxHit(c,
					// c.spellId), c.getCombat().getMagic(), defence);
					final boolean magicFailed = false;
					if (Misc.random(c.getCombat().mageAtk()) > Misc
							.random(defence))
						c.magicFailed = false;
					else
						c.magicFailed = true;
						...
						c.getPA()
								.addSkillXP(
										(c.MAGIC_SPELLS[c.oldSpellId][7] + damage
												* Config.MAGIC_EXP_RATE),
										6);
					}
					c.getItems();
					TaskHandler.submit(new Task(c.getCombat()
							.getHitDelay(
									i,
									ItemAssistant
											.getItemName(
								...
								public void execute() {
									try {
										c.getPA()
												.createPlayersStillGfx(
														failMage ? 85
																: c.MAGIC_SPELLS[spellId][5],
														c.objectX,
														c.objectY, 0, 1);
									} catch (Exception e) {
										this.cancel();
					...
					c.spellId = -1;
				} else if (c.usingBow || c.usingRangeWeapon) { // range
																// attack
					...
					try {
						c.getPA().createPlayersProjectile(pX, pY, offX,
								...
								c.getCombat().getRangeProjectileGFX(),
								43, 31, 0,
								c.getCombat().getProjectileShowDelay());
					} catch (Exception e) {
					}
					c.getPA().addSkillXP(
							(damage * Config.RANGE_EXP_RATE / 3), 4);
				} else {
					...
					if (c.fightMode == 3) {
						c.getPA()
								.addSkillXP(
										(damage * Config.MELEE_EXP_RATE / 3),
										0);
						c.getPA()
								.addSkillXP(
										(damage * Config.MELEE_EXP_RATE / 3),
	...
	public static void endGame() {
		...
		while (iterator.hasNext()) {
			...
			player.sendMessage("[@red@CASTLE WARS@bla@] Kills: @red@ "
					+ player.cwKills + " @bla@Deaths:@red@ " + player.cwDeaths
					+ "@bla@ Games Played: @red@" + player.cwGames + "@bla@.");
			player.getPA().createPlayerHints(10, -1);
			deleteGameItems(player);
	...
	public static void removePlayerFromCw(Client player) {
		...
		if (gameRoom.containsKey(player)) {
			...
			System.out
					.println("Deleting playername test: " + player.playerName);
	...
	public static void addCapes(Client player, int capeId) {
		...
		player.getEquipment().updateSlot(player.playerCape);
		player.appearanceUpdateRequired = true;
		Player.updateRequired = true;
	}
	...
	public static void addHoods(Client player, int hatId) {
		...
		player.getEquipment().updateSlot(player.playerHat);
		player.appearanceUpdateRequired = true;
		Player.updateRequired = true;
	...
	public static void deleteGameItems(Client player) {
		...
		for (int i = 0; i < items.length; i++) {
			if (player.getItems().playerHasItem(items[i])) {
				player.getItems().deleteItem2(items[i],
						player.getItems().getItemAmount(items[i]));
			}
		}
	}
	...
	public static int getZammyPlayers() {
		int players = 0;
		@SuppressWarnings("rawtypes")
		Iterator iterator = (!waitingRoom.isEmpty()) ? waitingRoom.values()
				.iterator() : gameRoom.values().iterator();
		while (iterator.hasNext()) {
	...
	public static void changeFlagObject(int objectId, int team) {
		new Object(objectId, FLAG_STANDS[team][0], FLAG_STANDS[team][1], 3, 0,
				10, objectId, -1, 1);
	}
}

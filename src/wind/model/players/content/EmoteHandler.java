...
/**
 * @Author - "TheLife" on Rune-Server and "TehLife" on Moparscape
 */
...
public class EmoteHandler {
	...
	public enum Emotes {
		Yes(168, 855, -1), No(169, 856, -1), Bow(164, 858, -1), Angry(167, 864,
				...
				-1), Zombie_Walk(72032, 3544, -1), Zombie_Dance(72033, 3543, -1), Scared(59062, 2836, -1), 
				Bunny_Hop(72254,
				6111, -1), Skillcape(74108, 1, 1);
	...
	public void startEmote(int buttonId) {
		...
		if (EMOTES != null && this.EMOTES == null) {
			this.EMOTES = EMOTES;
			if (EMOTES.animID != 1) {
				c.startAnimation(EMOTES.animID);
				c.usingEmote = true;
				if (EMOTES.gfxID != -1)
					c.gfx0(EMOTES.gfxID);
			} else {
	...
	public static int[][] skillcapeData = { { 9747, 823, 4959 },
			{ 9750, 828, 4981 }, { 9753, 824, 4961 }, { 9756, 832, 4973 },
			{ 9759, 829, 4979 }, { 9762, 813, 4939 }, { 9765, 817, 4947 },
			{ 9771, 830, 4977 }, { 9774, 835, 4969 }, { 9777, 826, 4965 },
			{ 9780, 818, 4949 }, { 9783, 812, 4937 }, { 9786, 827, 4967 },
			{ 9789, 820, 4953 }, { 9792, 814, 4941 }, { 9795, 815, 4943 },
			{ 9798, 819, 4951 }, { 9801, 821, 4955 }, { 9804, 831, 4975 },
			{ 9807, 822, 4957 }, { 9810, 825, 4963 }, { 9948, 907, 5158 },
			{ 9813, 816, 4945 }, };

	public static void doSkillcapeEmote(Client c) {
		if (c.playerEquipment[c.playerCape] == 9768
				|| c.playerEquipment[c.playerCape] == 9769) {
			c.gfx0(c.playerAppearance[0] == 0 ? 833 : 834);
			c.startAnimation(4971);
			c.usingEmote = true;
			return;
		}
		for (int i = 0; i < skillcapeData.length; i++) {
			if (c.playerEquipment[1] == skillcapeData[i][0]
					|| c.playerEquipment[1] == skillcapeData[i][0] + 1) {
				c.startAnimation(skillcapeData[i][2]);
				c.gfx0(skillcapeData[i][1]);
				c.usingEmote = true;
				c.stopMovement();

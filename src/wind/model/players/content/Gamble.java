...
public class Gamble {
	...
	private final int CHANCE_OF_WINNING = 65;

	@SuppressWarnings("unused")
	private static Client c;

	@SuppressWarnings("static-access")
	public Gamble(Client c) {
		this.c = c;
	}

	public Gamble(Client player, int cost) {
		if (canAfford(player, cost)) {
			if (calculateWin()) {
				player.getItems().addItem(995, cost);
				player.sendMessage("<col=2676846>Congratulations, you have won "
						+ cost + " coins!</col>");
				return;
			}
			player.sendMessage("<col=526584>Unlucky, you lose " + cost
					+ " coins. </col>");
	...
	public boolean canAfford(Client player, int amount) {

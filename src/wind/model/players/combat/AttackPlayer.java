...
public class AttackPlayer {
	...
	public static void applyPlayerMeleeDamage(Client c, int i, int damageMask,
		...
		if (damage > 0 && c.getEquipment().isWearingDharoks(c)) {
			if (Misc.random(4) == 1)
	...
	public static void playerDelayedHit(final Client c, final int i) {
		if (PlayerHandler.players[i] != null) {
			...
			if (!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				if (o.inMulti() && c.getCombat().multis() && NPCHandler.npcs[i].npcId != 3010) {
					c.barrageCount = 0;
	...
	public static void attackPlayer(Client c, int i) {
		//int k;
	//	Client o = (Client) PlayerHandler.players[c.playerId];
		Client o = (Client) PlayerHandler.players[i];
		...
		if (PlayerHandler.players[i] != null) {
			...
			if(c.isSkulled && c.inSafeZone()){
				Guard.constructGuard(c); // try that
				//c.underAttackBy = c.npcIndex;
				c.freezeTimer = 60;
			}
			/*
			 * Client c2 = (Client) PlayerHandler.players[i]; if
			 * (c.connectedFrom.equals(c2.connectedFrom)) {
			 * c.sendMessage("You cannot attack same ip address."); return; }
			 */
			...
			if (c.attackTimer <= 0) {
				...
				c.usingCross = c.playerEquipment[c.playerWeapon] == 15010;
				...
				if (c.getCombat().correctBowAndArrows() < c.playerEquipment[c.playerArrows]
						...
						&& c.playerEquipment[c.playerWeapon] != 15010
						&& !c.usingMagic) {
					c.getItems();
							c.getItems();
				...
				if (c.playerEquipment[c.playerWeapon] == 15010
						&& !c.getCombat().properBolts() && !c.usingMagic) {
					c.sendMessage("You must use bolts with a crossbow.");
					c.stopMovement();
					c.getCombat().resetPlayerAttack();
			...
			/*	public int skullTimer() {
					if (getRights().equal(Rights.DONATOR)) {
						return 2160;
					} else if (getRights().equal(Rights.SUPER_DONATOR)) {
						return 1920;
					} else if (getRights().equal(Rights.EXTREME_DONATOR)) {
						return 1680;
					} else if (getRights().equal(Rights.GOD_OF_ALL_DONATORS)) {
						return 1440;
					} else {
						return 2400;
					}
				...
				PlayerHandler.players[i].underAttackBy = c.playerId;
				//PlayerHandler.players[i].underAttackBy = c.npcIndex; // try that 
				PlayerHandler.players[i].logoutDelay = System
						.currentTimeMillis();
				PlayerHandler.players[i].singleCombatDelay = System
						.currentTimeMillis();
					...
					if (c.crystalBowArrowCount >= 250) {
						...
						case 4223: // 1/10 bow
							...
							if (!c.getItems().addItem(4207, 1)) {
								ItemHandler.createGroundItem(c, 4207,
